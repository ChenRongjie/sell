...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderDTO create(OrderDTO orderDTO) {
        ...
        orderMaster.setPayStayus(PayStatusEnum.WAIT.getCode());
    ...
    public OrderDTO cancel(OrderDTO orderDTO) {
        ...
        if (updateResult == null){
            log.error("【取消订单】 订单状态更新失败，orderMastr = {}",orderMaster);
        ...
        //如果已支付，需要退款
        if (orderDTO.getPayStayus().equals(PayStatusEnum.SUCCESS.getCode())){
            //TODO
        }
    ...
    public OrderDTO finish(OrderDTO orderDTO) {
        ...
        OrderMaster updateResult = orderMasterRepository.save(orderMaster);
        if (updateResult == null){
            log.error("【完结订单】 订单状态更新失败，orderMastr = {}",orderMaster);
            throw new SellException(ResultEnum.ORDER_UPDATE_FAIL);
        }
    ...
    public OrderDTO paid(OrderDTO orderDTO) {
        ...
        //判断支付状态
        if (!orderDTO.getPayStayus().equals(PayStatusEnum.WAIT.getCode())){
            log.error("【订单支付】 订单支付状态不正确, orderDTO = {}",orderDTO);
            throw new SellException(ResultEnum.ORDER_PAY_STATUS_ERROR);
        }

        //修改支付状态
        orderDTO.setPayStayus(PayStatusEnum.SUCCESS.getCode());
        OrderMaster orderMaster = new OrderMaster();
        BeanUtils.copyProperties(orderDTO,orderMaster);
        OrderMaster updateResult = orderMasterRepository.save(orderMaster);
        if (updateResult == null){
            log.error("【订单支付】 订单状态更新失败，orderMastr = {}",orderMaster);
            throw new SellException(ResultEnum.ORDER_UPDATE_FAIL);
        }
